...
| F1 | Local runtime | Installed daemon serves the loopback dashboard, the local MCP adapter is runnable, and the local socket is user-only | Shortcuts doctor and fresh source scan pass; strict MCP/RPC contract tests pass; preferred dashboard `10100` fell back to `10101` because another local service occupied it; dashboard/MCP Apps render native and app-owned rows with observe-only labels; socket mode 0600; the managed Tunnel runtime is ready and the gateway probe returns live data | PASS |
...
- The managed Tunnel runtime is healthy after the operator restored the
  protected `CONTROL_PLANE_API_KEY` in the launchd user environment; the key
  remains outside the repository, profile, Registry, and logs.
